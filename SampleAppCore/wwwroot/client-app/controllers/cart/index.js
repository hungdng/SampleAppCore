var CartController = function () {
    ...
    function registerEvents() {
        $('body').on('click', '.btn-delete', function (e) {
            ...
            $.ajax({
                ...
                success: function () {
                    sampleCore.notify('Removing product is successful.', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
        ...
        $('body').on('keyup', '.txtQuantity', function (e) {
            ...
            if (q > 0) {
                $.ajax({
                    ...
                    success: function () {
                        sampleCore.notify('Update quantity is successful', 'success');
                        loadHeaderCart();
                        loadData();
                    }
                });
            } else {
                sampleCore.notify('Your quantity is invalid', 'error');
            }
        ...
        $('#btnClearAll').on('click', function (e) {
            ...
            $.ajax({
                ...
                type: 'post',
                success: function () {
                    sampleCore.notify('Clear cart is successful', 'success');
                    loadHeaderCart();
                    loadData();
                }
            });
    ...
    function loadColors() {
        return $.ajax({
            ...
            error: function () {
                sampleCore.notify('Has an error in progress', 'error');
            }
        });
    }
                    ...
                    render += Mustache.render(template,
                        {
                            ...
                            Amount: sampleCore.formatNumber(item.Price * item.Quantity, 0),
                            Url: '/' + item.Product.SeoAlias + "-p." + item.Product.Id + ".html"
                        });
                    totalAmount += item.Price * item.Quantity;
                });
                $('#lblTotalAmount').text(sampleCore.formatNumber(totalAmount, 0));
                if (render !== "")
